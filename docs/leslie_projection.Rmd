...
#create empty matrix to store population sizes
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

#populating first row with year 0 population sizes
proj_year[1, ] <- fish_year0

#estimate population size for each year based on the population size in the previous year
for (i in 2:length(time_yr)) {
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,]
}

for (i in 2:length(time_yr)) {
  proj_year[i,] <- fish_leslie %*% proj_year[i-2,]
}

# The rest is wrangling & visualization (run to check it out):
#updating columns name for the population stages
